...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 지금까지 배운 것 '=' 대입연산자의 양 옆 자료형(타입)이 같아야 한다.

		// 1. 부모타입 래퍼런스로 부모 객체를 다루는 경우

		System.out.println("1. 부모타입 래퍼런스로 부모 객체를 다루는 경우");
		Parent p1 = new Parent();
		p1.printParent(); // p1 레퍼런스로 Parent 객체에 접근 가능

		// 2. 자식타입 레퍼런스(참조변수)로 자식객체를 다루는 경우

		System.out.println("2. 자식타입 레퍼런스(참조변수)로 자식객체를 다루는 경우");
		Child1 c1 = new Child1();
		c1.printParent();
		/*((Parent)c1)*/c1.printParent();
		c1.printChild1();
		// c1 레퍼런스로 Parent, Child1 둘 다 접근가능(Parent 접근시 자동으로 형변환 되어 진행이 되었던것)

		// 다형성
		// 3. 부모타입의 레퍼헌스로 자식 객체를 다루는 경우
		System.out.println("3. 부모타입의 레퍼헌스로 자식 객체를 다루는 경우");

		Parent p2 = /*(Parent)*/ new Child1(); // 자동으로 형변환 되고있는 것

		p2.printParent();
		// p2 레퍼런스는 Parent만 접근가능하다
		// 하지만 Child 접근 하고싶으면 형변환을 해야함 (명시적(강제)형변환)
		...
		// 3. 불가능한 경우 : 다형성의 정의는 부모타입으로부터 파생된 여러가지 타입의 자식객체를 부모 클래스타입 하나로 다룰 수 있는 기술
//		Child1 c2 = new Parent(); // 컴파일 에러 : 소스상의 문법 오류(타입이 맞지 않아서 에러 발생)
//		Child1 c2 = (Child1)new Parent();
//		c2.printParent(); // 런타임(실행) 오류 : 프로그램 실행했을 때 오류(부모객체는 자식으로 받아줄 수 없음)
		...
		// 다형성이 적용된 경우 - 하나의 부모타입 레퍼런스로 다양한 자식 객체들을 다 받을 수 있다. (코드길이 감소, 유연한 코딩가능)
