...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println(bk1.toString()); // 재정의를 하지 않았기 때문에 Object에서 불러온거나 마찬가지
		System.out.println(bk1/*.toString()*/); // 출력문에 레퍼런스 출력 시 JVM이 자동으로 레퍼런스.tostring() 호출해줌 (생략된거나 마찬가지)

		// 모든 클래스는 object 클래스의 후손이다. 즉, 최상위 클래스는 object이다.
		// => Object 에 있는 모든 메소드들은 재정의를 할 수 있다.

		// 1.toString()
		// 오버라이딩 전 : Object 클래스의 toString()이 실행 --> 풀패키지명 @ 해쉬코드 16진수 값이 리턴
		// 오버라이딩 후 : Book 클래스에 있는 toString()실행 --> 내가 재정의 한대로 해당 객체가 가지고있는 필드값에 대한 정보를
		// 리턴.

		System.out.println(bk2);

		// -------------------------------------equals(),hashCode() 둘이 같이 바꿔줘야함
		// 2.equals()-------------------------------------------------------------------------------------

		Book bk3 = new Book("언어의 온도", "이기주", 20000); // bk1과 동일한 필드값을 가진 bk3 객체를 생성 (주소값이 다르다)
