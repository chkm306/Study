...
public class Run {
	...
	public static void main(String[] args) {
		...
		List a1 = new ArrayList(); // 다형성 적용
		
		//add() : ArrayList 데이터 추가
		//add에 마우스를 올리면 Object 타입으로 들어가는 것을 볼 수 있는데 이것은 컬렉션은 객체타입만 받아주기 때문에
		//기본 자료형을 추가하면 내부적으로 Autoboxing에 의해 Wrapper 클래스로 변형되어 객체로 들어간다.
		a1.add(true);
		a1.add("이효리");
		a1.add('여');
		a1.add(20);
		...
		//get(index) : ArrayList 데이터 추출
		for(int i = 0; i < a1.size(); i++) {
			System.out.println("인덱스 " + i + "번째 값 : "+a1.get(i));
		}
		System.out.println(a1.toString());
		
		// remove(인덱스) : 원하는 인덱스 번째값 삭제
		a1.remove(1);
		System.out.println("1인덱스 삭제 후 : "+a1);
		
		// add() : 중간에 값 추가, 원하는 인덱스 번쨰
		a1.add(3, 36.78);
		System.out.println("3번 인덱스 추가 후 : "+a1);
		
		// set(인덱스, 변경할 값) : 원하는 인덱스 번째 값 수정
		a1.set(3, "abc");
		System.out.println("3번 인덱스 수정 후 : "+a1);
		
		// IndexOf(찾을값) : 찾고자하는 값의 인덱스 확인
		System.out.println(a1.indexOf("abc"));
		
		// contains(찾을값) : 찾을 값을 포함하고 있는지 (Boolean)
