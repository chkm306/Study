...
// 변수 위치에 따르느 구분을 보기 위한 클래스 (전역, 지역, 매개)
...
public class FieldTest1 {
	...
	public void test(int num) { // num -> 매개변수 (메소드 괄호 안에 작성하는 변수), 일정의 지역변수, 이 메소드 안에서만 사용가능
		
		// 지역 변수(특정구역, 즉 메소드 영역 내에서 작성하는 변수
		int local = 10; // 지역변수는 접근 제한자가 붙지 않는다.
		
		// 멤버 변수 출력 --> 전역 변수는 클래스 전역에서 사용 가능하고 JVM이 기본값으로 초기화한다.
		System.out.println(global);
		
		// 지역 변수는 반드시 초기화 하자
		System.out.println(local);
		
		// 매개 변수는 메소드 호출 시 반드시 값이 전달되어 온다.
