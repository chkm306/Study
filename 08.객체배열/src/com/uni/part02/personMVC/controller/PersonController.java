...
public class PersonController {
	private Person[] people = new Person[3]; // 세명의 인원, 전역변수
	
	private int count = 0; // 현재 추가된 사람 수를 나타낼 변수

	public boolean insertPerson(String name, int age, int wealth) {

		if (count < people.length) { // 3보다 작을때까지
			people[count++] = new Person(name, age, wealth); // 0명부터 시작, 후증가
			return true; // 에러뜸 반환한다고?? ㄹㅇ?
		} else {
			return false;
		}
	}
	...
	public Person searchPerson(String search) {
		for(int i = 0; i < count; i++) {
			if(people[i].getName().equals(search)) {
				return people[i]; // 인덱스에 있는 사람의 정보를 넘긴다
			}
		}
		return null; // 객체에 아무것도 없을때는 null 던져라
	}

	public String getavgWealth() {
		
		int sum = 0; // 변수에 합이 담긴다
		for(int i = 0; i < count; i++) {
