...
public class A_UnCheckException {
	...
	public void method1() {
		...
		// int result = num1 / num2; // 10을 0으로 나눴을때 비정상 종료
		// Exception in thread "main" java.lang.ArithmeticException: / by zero
		int result = 0;

		if (num2 != 0) { // 애초에 예외가 발생할 일이 없게 if문으로 조건검사
			result = num1 / num2;
		}
	...
	public void method3() {
		...
		//String str = (String)obj; // 컴파일에러X 런타임에러O
...
//		arr[2] = 3; // 런타임에러O
		 ...
		 // - NullPointerException : 참조변수가 null로 초기화 된 상황에서 메소드를 호출하는 예외상황 --> 개발자가 실수 (코드 해결하면됨)
		String str1 = null;
		int length = str1.length(); // 런타임 에러 O
		
	/*}catch(ClassCastException e){
		System.out.println("잘못 형변환했네");
	}catch(ArrayIndexOutOfBoundsException e) {
		System.out.println("배열 크기를 잘못 지정했네");
	}catch(NullPointerException e) {
		System.out.println("객체없이 메소드를 쓰려고 하네");
	}*/
		// Multi catch문 --> 나란히 작성하는 경우 형제 레벨만 함께 사용
