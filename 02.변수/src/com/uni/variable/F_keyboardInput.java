...
public class F_keyboardInput {
	...
	public void inputScanner1() {
		...
		Scanner sc = new Scanner(System.in); // --> 입력받은 값을 바이트 단위로 받아들이겠다.
		
		System.out.println("당신의 이름은 무엇입니까 ? ");
		
//		String name = sc.next(); // 사용자가 입력한 문자열에서 공백이 있는 경우 공백 이전까지 읽어온다. 즉, 띄어쓰기 전까지
		String name = sc.nextLine(); // 사용자가 입력한 값 모두 읽어온다.
		
		System.out.println("당신의 나이는 몇 살입니까 ? ");
		
		int age = sc.nextInt(); // 사용자가 입력한 정수 값만 읽어온다.
		
		System.out.println("당신의 키는 몇 입니까?");
		
		double height = sc.nextDouble(); //사용자가 입력한 실수 값만 읽어온다
	...
	public void inputScanner2() {
		 ...
		 * 사용자가 값을 입력했다고 생각한 후 읽어오고 넘어가버림 (처음 오류예시)
		 * 
		...
		System.out.println(name+ " 님은 주소는 "+ address +"이고 "+ age + " 세이며, 키는 " + height + " cm 입니다.");
	...
	public void inputScanner3() {
		...
		//정수형을 입력 받을 때 -> sc.nextInt()
		//실수형을 입력 받을 때 -> sc.nextDouble()

//		sc.nextChar() -> 존재하지 않음. CharAt() : 문자열에서 지정된 위치에 있는 문자 하나만 뽑아내는 기능을 함. 
...
//		a.charAt(0);
		...
		char gender = sc.nextLine().charAt(0); //String에 있고, 몇번째 문자열부터 불러올지
		 ...
		 * 그 뒤에 .charAt(0) 메소드를 통해(인덱스로 접근해서) 문자로 변환해주는 작업 진행해야된다.
		 * 
		 */
