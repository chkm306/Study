...
public class A_Variable {
	...
	public void declareVariable() {
		...
		byte bNum; //1byte
		short sNum; //2byte
		int iNum;   //4byte --> 정수형 중에 가장 대표적인 자료형/기본형 -21억~~~양수 21억.xxx까지 값이 담기는 자료형
		long lNum;  //8byte
		
		//2-2 실수형
		
		float fNum; //4byte // 소수점 7자리까지 가능
		double dNum;//8byte --> 실수형 중에 가장 대표적인 자료형/기본형 소수점 15자리까지 가능
		...
		bNum = 1; //커서올리면 위치 다 나옴
		sNum = 2;
		iNum = 5;
		lNum = 8L; //구분하기위해서 L을 붙임
		
		fNum = 4.0f; //반드시 f를 붙여야함 (이해안됨)(기본으로 더블형으로 읽는다는것도 이해안댐)(그럼얘가필요함?)
		dNum = 8.0;	//d를 붙이지 않아도 된다.
		
		ch = 'A'; //따옴표를 사용해서 값 대입
		//ch = 'ABC'; //문자에는 한 문자가 저장됨.
		
		ch = '강'; //유니코드화 되기때문에 문자열 어쩌구 8:45
		 ...
		 * */
			...
			byte bNum = 1; //커서 올리면 위치 다 나옴
			short sNum = 2;
			int iNum = 5;
			long lNum = 8L; //구분하기위해서 L을 붙임
			
			float fNum = 4.0f; //반드시 f를 붙여야함 (이해안됨)(기본으로 더블형으로 읽는다는것도 이해안댐)(그럼얘가필요함?)
