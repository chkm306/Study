...
public class D_Cast {
	...
	public void rule1() {
		/* 자동 형변환 :
		 * 값의 범위가 작은 자료형과 큰 자료형의 연산(대입, 계산 등)을 컴파일러가 자동으로 범위가 작은 자료형을 큰 자료형으로 변환 처리 해준다.
		 * 
		 */
		...
		int i2 = 12;
		double d2 = 3.3;
		double result = i2 + d2; // (double) 생략된거. 12.0+3.3 => 15.3
		System.out.println("result : "+ result); //int형과 double 형의 연산의 결과가 double형으로 자동으로 변환
		...
		//크기는 long 형이 더 크지만 float 형으로 자동 형변환이 가능한 이유는 표현 가능한 수의 범위가 float형이 더 크기때문.
		long l6 = 1000000000L;
		float f6 = l6; //선언했지만 안 사용하면 노란줄뜸
		...
		//char 의 자료형에 값(숫자)이 들어오면 해당 숫자와 일치하는 문자를 유니코드에서 찾아서 출력한다. 4:51
...
//		ch = -65; char 형은 음수값 저장 불가 : 값의 범위가 0~65535 까지이기 때문
		...
		int result3 = b2 + b3; //==> byte 나 short 연산 시 무조건 int로 처리, 따라서 강제 형변환이 필요하다.
	...
	public void rule2() {
		...
		//강제 형변환 : 큰 크기의 자료형을 작은 크기의 자료형으로 바꾸는 것 (형변환 생략이 불가)
...
//		int iSum = iNum + dNum; // 10.0 + 5.89 -> 15.89 (double 형 -> int형 변수로 대입 불가)
		
		//해결방법
		//1. 수행결과를 int 형으로 강제 형변환 //15
		int iSum = (int)(iNum + dNum); // 실수 -> 정수 : 강제형변환 발생시 소수점 아래부분이 버려짐.
		System.out.println(iSum);
		
		
		//2. double 형 값을 int 형으로 강제 형변환 //15
		int iSum2 = iNum + (int)dNum;
		System.out.println(iSum2);
		
		
		//3. 연산결과를 double 형으로 받음. //15.89
...
//		byte result = bNum + sNum;
		...
		// 방법 1 : 연산결과를 byte나 short로 받고 강제 형변환
		...
		System.out.println(temp2); //계산하기 쉽지 않은 데이터 손실이 발생되므로 주의해서 사용해야한다.
	...
	public void stringTest() {
		...
		// 리터럴 : stringpool 에서는 이미 값이 있으면 주소를 생성하지 않는다.
		// new : 무조건 힙에 새로운 인스턴스 객체(스트링풀 공간) 생성, 반환타입이 참조형이라 하는것은 반환되는 값이 객체의 주소를 반환한다는 것을 의미.
		
		
		
		String s = "안녕하세요"; // heap (stringpool)
		String s1 = "안녕하세요"; // heap (stringpool) 이미 존재
		...
		String t = "정지훈"; // heap (stringpool) 생성
		String t1 = "이효리"; // heap (stringpool) 생성
		String t2 = new String("유재석"); // heap 생성
		String t3 = "유재석"; // heap (stringpool) 생성
		String t4 = "이효리"; // heap (stringpool) 이미 존재
